...
@app.get(
        ...
        summary="Get class by course name"
)
def coursename(course_name: str, db: Session = Depends(get_db)):
    return crud.get_class_by_course_name(db, course_name)
    
@app.get(
    "/course_catalogue/{course_number}",
    summary="Get Course catalogue information for a singular class",
)
def course_catalogue_search(course_number: str, db: Session = Depends(get_db)):
